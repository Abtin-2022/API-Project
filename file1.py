...
if __name__ == "__main__":
    ...
    while (True):
        ...
        user_categories = ["category", "history", "general", "exit"]
        ...
        response1 = gemini_query(
            f"From now on you are a professional suggester. Can you categorize the user's prompt, which is: {user}. According to this list of categories: {user_categories}? Only output `category` if the user asks for one of the following categories in news: {categories}, catch misspelled words. Give just one word. If the user asks for their history such as `I want to retrieve all my past articles`, `i want to get all articles regarding...`, `i want my news [category] history`, or `I want to retrieve my history about [category]` or 'I want to retrieve the history of articles relating to [category]' then output `history`. If the user asks for something else, then output `general`. If the user asks for `exit` or to exit the program, then output `exit`. Do not output anything else, just the single word.",
        )
